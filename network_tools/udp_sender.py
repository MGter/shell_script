...
def printHelp():
    print("""
用法: python udp_sender.py <输入文件> <IP地址> <端口> <码率> [循环次数]
示例: python udp_sender.py 50_audio_program_300s.ts 127.0.0.1 25234 16600000 2
...
参数:
  ...
  端口: 目标端口号
  码率: 以比特每秒为单位的码率
  循环次数: 发送循环次数，默认为1，-1表示发送一次，>0表示循环指定次数
""")
    sys.exit(1)

def send_udp_with_bitrate(input_file, ip, port, bitrate, loop=1, packet_size=1316):
    try:
        # 验证输入参数
        ...
        for _ in range(loop_count):
            ...
            print(f"完成第{_ + 1}次向{ip}:{port}发送{input_file}，码率为{bitrate} bps")
...
if __name__ == "__main__":
    ...
    try:
        ...
        loop = int(sys.argv[5]) if len(sys.argv) == 6 else 1
        
        # 执行UDP发送函数
        send_udp_with_bitrate(input_file, ip, port, bitrate, loop)
